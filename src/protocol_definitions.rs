...
impl TryFrom<&GetFwVersionResponse> for [u8; 60] {
    ...
    fn try_from(response: &GetFwVersionResponse) -> Result<Self, Self::Error> {
        ...
        for i in 0..response.header.component_count as usize {
            let component = &response.component_info.get(i).ok_or(ConversionError::ByteConversionError)?;
            ...
            *bytes.get_mut(offset + 1).ok_or(ConversionError::ByteConversionError)? = component.component_id;
            bytes.get_mut(offset + 2..offset + 4).ok_or(ConversionError::ByteConversionError)?.copy_from_slice(&component.vendor_specific1.to_le_bytes());
            *bytes.get_mut(offset + 4).ok_or(ConversionError::ByteConversionError)? = component.fw_version.major;
            bytes.get_mut(offset + 5..offset + 7).ok_or(ConversionError::ByteConversionError)?.copy_from_slice(&component.fw_version.minor.to_le_bytes());
...
impl TryFrom<&[u8; 60]> for GetFwVersionResponse {
    ...
    fn try_from(bytes: &[u8; 60]) -> Result<Self, Self::Error> {
        ...
        let _reserved = u16::from_le_bytes(bytes[1..3].try_into().map_err(|_| ConversionError::ByteConversionError)?);
        ...
        for component in component_info.iter_mut().take(component_count as usize) {
            ...
            component.vendor_specific1 = u16::from_le_bytes(bytes[offset + 2..offset + 4].try_into().map_err(|_| ConversionError::ByteConversionError)?);
            component.fw_version.major = bytes[offset + 4];
            component.fw_version.minor = u16::from_le_bytes(bytes[offset + 5..offset + 7].try_into().map_err(|_| ConversionError::ByteConversionError)?);
...
impl TryFrom<&[u8; 32]> for FwUpdateOffer {
    ...
    fn try_from(bytes: &[u8; 32]) -> Result<Self, Self::Error> {
        ...
        let firmware_version = FwVersion {
            major: bytes[7],
            minor: u16::from_le_bytes(bytes[5..7].try_into().map_err(|_| ConversionError::ByteConversionError)?),
            variant: bytes[4],
        };

        let vendor_specific = u32::from_le_bytes(bytes[8..12].try_into().map_err(|_| ConversionError::ByteConversionError)?);
        let misc_and_protocol_version = u32::from_le_bytes(bytes[12..16].try_into().map_err(|_| ConversionError::ByteConversionError)?);
...
impl TryFrom<&[u8; 60]> for FwUpdateContentCommand {
    ...
    fn try_from(bytes: &[u8; 60]) -> Result<Self, Self::Error> {
        ...
        let sequence_num = u16::from_le_bytes(bytes[2..4].try_into().map_err(|_| ConversionError::ByteConversionError)?);
        let firmware_address = u32::from_le_bytes(bytes[4..8].try_into().map_err(|_| ConversionError::ByteConversionError)?);
