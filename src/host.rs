...
impl<W: CfuWriterAsync> CfuUpdateContent<W> for CfuUpdater {
    ...
    async fn write_data_chunks(
        ...
        let mut resp: FwUpdateContentResponse = FwUpdateContentResponse::new(0, CfuUpdateContentResponseStatus::ErrorInvalid);
        for i in 0..num_chunks {
            ...
            let r = match i {
                ...
                _ => {
                    image
                        .get_bytes_for_chunk(chunk.get_mut(0..remainder).ok_or(CfuProtocolError::WriterError(CfuWriterError::Other))?, address_offset)
                        .await
                        .map_err(|_| CfuProtocolError::WriterError(CfuWriterError::StorageError))?;
